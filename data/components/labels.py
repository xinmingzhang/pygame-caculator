...
BUTTON_DEFAULTS = {
        ...
        "hover_image": None,
        "disable_image": None,
        'click_fill_color':None,
        'click_text_color':None,
        'click_text':None,
        'click_image':None,
        "hover_sound": None,
        "click_sound": None,
...
class Label(pg.sprite.Sprite, tools._KwargMixin):
    ...
    """
    def __init__(self, text, rect_attr, **kwargs):
        """
...
class MultiLineLabel(pg.sprite.Sprite):
    ...
    def __init__(self, path, size, text, color, rect_attr, bg=None,
                         char_limit=30, align="left", vert_space=0):
        """
        ...
        """
        attr = {"center": (0, 0)}
        lines = [text[i*char_limit:(i+1)*char_limit] for i in range(math.ceil(len(text)/char_limit))]
        arguments ={'font_path':path,
                    'font_size':size,
                    'text_color':color,
                    'fill_color':bg}
        labels = [Label(line,attr,**arguments) for line in lines]
...
class Button(pg.sprite.Sprite, tools._KwargMixin):
    ...
    def __init__(self, topleft, *groups, **kwargs):
        ...
        color_args = ("text_color", "hover_text_color", "disable_text_color",
                      ...
                      'click_fill_color','click_text_color')
        ...
        self.disable_image = self.make_image(self.disable_fill_color,
                                             self.disable_image,
                                             rendered["disable"])     
        self.click_image = self.make_image(self.click_fill_color,
                                           self.click_image,
                                           rendered['click'])
    ...
    def render_text(self):
        ...
        click = self.click_text and self.font.render(self.click_text, 1,
                                                       self.click_text_color)
        return {"text": text, "hover": hover, "disable": disable, 'click':click}
    ...
    def get_event(self, event):
        ...
        if self.active and self.visible:
            ...
            elif event.type == pg.KEYDOWN and (event.key in self.bindings and event.unicode == self.args):
